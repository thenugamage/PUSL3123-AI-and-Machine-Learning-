%% ================================
%  PUSL3123 - SCRIPT 1 (FINAL)
%  Pre-processing + Segmentation + Feature Extraction + Variability/PCA
%
%  Coursework scenario:
%   1) Preprocess acceleration/gyro data from smartphones
%   2) Extract window-based features (2s, 50% overlap)
%   3) Build reference (FD) and testing (MD) templates
%   4) Analyse variability/uniqueness across users (for report)
%
%  Expected input filenames (in data\raw_data):
%    U1NW_FD.csv, U1NW_MD.csv, ..., U10NW_FD.csv, U10NW_MD.csv
%
...
%    data\processed\testing_templates.(mat/csv)
%    data\processed\feature_variability_top.csv
%    data\processed\feature_means_per_user.csv
%    data\processed\analysis\*  (PCA plots + extra variability CSVs)
% =================================

clc; clear; close all;

%% --------- PROJECT / DATA FOLDER SETUP ----------------------------------
% Hard-coded project root as you specified.
% Change this ONE line if you move the project:
projectRoot = fileparts(mfilename('fullpath'));

% Raw data folder (your provided path)
rawFolder      = fullfile(projectRoot, 'data', 'raw_data');

% Interim + processed folders
...
% Clear old CSVs / templates for a clean run
...
delete(fullfile(analysisFolder, '*.csv'));
delete(fullfile(analysisFolder, '*.png'));
delete(fullfile(analysisFolder, '*.mat'));

% Output paths for feature/template MAT/CSV
...
%% --- Config -------------------------------------------------------------
forceFs         = 31;   % Hz - forced sampling frequency for all files
fc              = 10;   % Hz - low-pass cutoff (captures gait dynamics, removes noise)
windowLengthSec = 2.0;  % seconds - window length
overlapFraction = 0.5;  % 50% overlap

% Channels order for filtered + z-score signals
sensorNames = {'AccX_f','AccY_f','AccZ_f','GyroX_f','GyroY_f','GyroZ_f'};
numSensors  = numel(sensorNames);

% Global z-score accumulators (filtered signals only)
...
fprintf('\n========================================\n');
fprintf('  SCRIPT 1 (FINAL): PRE-PROCESSING + FEATURES + ANALYSIS\n');
...
for k = 1:numel(fileList)
    ...
    fprintf('\n[PASS 1: %d/%d] Raw file: %s\n', k, numel(fileList), filename);
    fprintf('----------------------------------------\n');

    filepath = fullfile(rawFolder, filename);

    % Load raw table (no header row in file)
    ...
    if any(any(ismissing(raw))) || any(any(isnan(table2array(raw))))
        fprintf('    ⚠ Missing data detected → linear interpolation\n');
    ...
    %% STEP 2: Sort time, merge duplicate timestamps, resample to forceFs
    fprintf('  STEP 2: Timestamp handling + resampling to %d Hz\n', forceFs);

    % Sort by time
    [tSorted, sortIdx] = sort(raw.Time);
    dataMat = [raw.AccX, raw.AccY, raw.AccZ, ...
               raw.GyroX, raw.GyroY, raw.GyroZ];
    dataMat = dataMat(sortIdx,:);

    % Merge duplicate timestamps by averaging
    [tUnique, ~, ic] = unique(tSorted);
    if numel(tUnique) < numel(tSorted)
        fprintf('    ⚠ Duplicate timestamps → averaging duplicates\n');
        Xuniq = zeros(numel(tUnique), size(dataMat,2));
    ...
    approxFs = (numel(tSorted) - 1) / (tEnd - t0 + eps);
    targetFs = forceFs;
    fprintf('    Approx Fs in file: %.2f Hz → using forced Fs = %d Hz\n', ...
            approxFs, targetFs);

    % Uniform time grid & interpolation
    t_resampled = (t0 : 1/targetFs : tEnd).';
    data_resampled = interp1(tSorted, dataMat, t_resampled, 'linear', 'extrap');
    ...
    fprintf('  STEP 3: 4th-order Butterworth low-pass (fc=%.1f Hz)\n', fc);

    Wn = fc / (targetFs/2);
    if Wn >= 1
        error('Cutoff fc=%.2f Hz is too high for Fs=%.2f Hz (Wn=%.2f).', ...
              fc, targetFs, Wn);
    end
    [b, a] = butter(4, Wn, 'low');
    ...
    fprintf('  STEP 4: Save filtered + accumulate global stats\n');
...
% Save normalisation parameters if you want to reuse them later
save(fullfile(processedRoot, 'normalisation_params.mat'), ...
     'globalMean','globalStd','sensorNames');

%% ========================================================================
%%  PASS 2: FILTERED -> NORMALISED + SEGMENTATION + FEATURES
%% ========================================================================

% Storage for ALL features (FD + MD) for model training later
allFeatures    = [];   % [N_all x N_features]
allUserID      = [];   % numeric user ID (1..10)
allDayType     = [];   % 1 = FD, 2 = MD, 0 = unknown
...
fprintf('  Filtered files: %d\n', numel(filteredList));
fprintf('========================================\n');

for f = 1:numel(filteredList)
    fname = filteredList(f).name;
    fprintf('\n[PASS 2: %d/%d] Filtered file: %s\n', f, numel(filteredList), fname);
    ...
    sensorZVars = {'AccX_f_z','AccY_f_z','AccZ_f_z', ...
                   'GyroX_f_z','GyroY_f_z','GyroZ_f_z'};
...
featuresTable.dayType    = allDayType;      % 1 = FD, 2 = MD, 0 = unknown
...
fprintf('\nSaved ALL features to:\n  %s\n  %s\n', featuresMatPath, featuresCsvPath);

%% --- Feature variability & uniqueness across users (ALL windows) --------
fprintf('\n========================================\n');
fprintf('  GLOBAL FEATURE VARIABILITY ANALYSIS (FD + MD)\n');
fprintf('========================================\n');
...
if numUsers < 2
    ...
    % Per-user mean and variance across ALL segments
    ...
    betweenVar = var(userMeans, 0, 1, 'omitnan');  % user means difference
    withinVar  = mean(userVars, 1, 'omitnan');     % intra-user variance
    discRatio  = betweenVar ./ (withinVar + eps);  % higher = more discriminative
    ...
    % Save per-user mean feature vectors
...
%% ---------- PART C: VARIABILITY & PCA ANALYSIS (FD only, for report) ----
if isempty(refFeatures)
    fprintf('\n[Analysis] No reference features available – skipping FD-only analysis.\n');
else
    fprintf('\n=== ANALYSING FEATURE VARIABILITY / PCA (FD templates only) ===\n');

    X       = refFeatures;       % [numWindows x numFeatures]
    userIDs = refUserID_cat;     % categorical labels
    featNm  = featureNames;

    userList        = categories(userIDs);
    nUsersFD        = numel(userList);
    nFeatFD         = size(X,2);
    userMeansFD     = zeros(nUsersFD, nFeatFD);
    userStdsFD      = zeros(nUsersFD, nFeatFD);
    nWindowsPerUser = zeros(nUsersFD, 1);

    for i = 1:nUsersFD
        idx = (userIDs == userList{i});
        nWindowsPerUser(i) = sum(idx);
        userMeansFD(i,:)   = mean(X(idx,:), 1);
        userStdsFD(i,:)    = std(X(idx,:), 0, 1);
    end

    % Print how many windows per user
    fprintf('\nWindows per user (FD only):\n');
    for i = 1:nUsersFD
        fprintf('  %s: %4d windows\n', string(userList{i}), nWindowsPerUser(i));
    end

    % Pairwise distances between user centroids (how separable the users are)
    D = zeros(nUsersFD);
    for i = 1:nUsersFD
        for j = i+1:nUsersFD
            d = norm(userMeansFD(i,:) - userMeansFD(j,:));
            D(i,j) = d;
            D(j,i) = d;
        end
    end

    fprintf('\nPairwise distances between user mean feature vectors (FD):\n');
    fprintf('  Min distance (excluding 0): %.3f\n', min(D(D>0)));
    fprintf('  Max distance:               %.3f\n', max(D(:)));

    % Between-user variance vs total variance per feature (FD windows)
    totalVarFD   = var(X, 0, 1);          % variance across all FD windows
    betweenVarFD = var(userMeansFD, 0, 1);% variance of FD user means
    ratioFD      = betweenVarFD ./ (totalVarFD + eps);  % bigger -> more discriminative

    [ratioSorted, idxSorted] = sort(ratioFD, 'descend');
    topK_fd = min(10, numel(idxSorted));

    fprintf('\nTop %d most user-discriminative features (FD only, betweenVar/totalVar):\n', topK_fd);
    for k = 1:topK_fd
        fprintf('  %2d) %-20s  ratio = %.3f\n', ...
            k, featNm{idxSorted(k)}, ratioSorted(k));
    end

    % Save per-user statistics to CSV
    userStatsTable = array2table(userMeansFD, 'VariableNames', featNm);
    userStatsTable.UserID = userList;
    userStatsPath = fullfile(analysisFolder, 'feature_variability_users_FD.csv');
    writetable(userStatsTable, userStatsPath);

    % Save feature-level ratios to CSV
    featureStatsTable = table(featNm', ratioFD', ...
        'VariableNames', {'Feature','BetweenToTotalVar'});
    featureStatsPath  = fullfile(analysisFolder, 'feature_variability_features_FD.csv');
    writetable(featureStatsTable, featureStatsPath);

    % PCA visualisation (2D) if available
    if exist('pca','file') == 2
        fprintf('\nRunning PCA for visualisation of FD user separation...\n');
        muFD  = mean(X, 1);
        sigFD = std(X, 0, 1);
        XzFD  = (X - muFD) ./ (sigFD + eps);

        [coeff, score, ~] = pca(XzFD); %#ok<ASGLU>
        figure;
        gscatter(score(:,1), score(:,2), userIDs);
        xlabel('PC 1'); ylabel('PC 2');
        title('User Separation in Feature Space (PCA, FD windows only)');
        grid on;

        pcaPath = fullfile(analysisFolder, 'feature_pca_users_FD.png');
        saveas(gcf, pcaPath);
        fprintf('  -> PCA scatter plot saved to %s\n', pcaPath);
        close(gcf);
    else
        fprintf('\nPCA not available – skipping PCA plot.\n');
    end

    % Save analysis struct
    analysis.userList         = userList;
    analysis.nWindowsPerUser  = nWindowsPerUser;
    analysis.userMeansFD      = userMeansFD;
    analysis.userStdsFD       = userStdsFD;
    analysis.distanceMatrix   = D;
    analysis.featureNames     = featNm;
    analysis.betweenToTotalFD = ratioFD;

    analysisMatPath = fullfile(analysisFolder, 'feature_variability_analysis_FD.mat');
    save(analysisMatPath, 'analysis');

    fprintf('\n[Analysis] Saved FD-only variability/uniqueness results to:\n');
    fprintf('  MAT : %s\n', analysisMatPath);
    fprintf('  CSV (users)   : %s\n', userStatsPath);
    fprintf('  CSV (features): %s\n', featureStatsPath);
    fprintf('=== END OF VARIABILITY & PCA ANALYSIS ===\n');
end
