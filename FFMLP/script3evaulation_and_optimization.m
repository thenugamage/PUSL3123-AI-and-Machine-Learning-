...
%   - data/processed/trained_patternnet_B_fd_md.mat (Script 2, Method B)
%
...
%% -------------------------- PATHS ---------------------------------------
% If this script is inside src/, then use fileparts(fileparts(...))
% If it sits in the project root (same level as /data), use single level:
projectRoot   = fileparts(mfilename('fullpath'));  
processedRoot = fullfile(projectRoot, 'data', 'processed');

featuresMatPath = fullfile(processedRoot, 'features_all.mat');
modelFDMDPath   = fullfile(processedRoot, 'trained_patternnet_B_fd_md.mat');
...
if ~exist(modelFDMDPath, 'file')
    error('trained_patternnet_B_fd_md.mat not found at:\n  %s\nRun Script 2 (Method B) first.', modelFDMDPath);
end
...
load(modelFDMDPath, 'net_B', 'muX_B', 'sigmaX_B', ...
                    ...
                    'maskFeat', 'featureNames_sel');

X_all   = allFeatures;
y_all   = allUserID;
day_all = allDayType;

% If Script 2 dropped near-zero-variance features, apply the same mask here
if exist('maskFeat','var') && ~isempty(maskFeat)
    X_all       = X_all(:, maskFeat);
    featureNames = featureNames(maskFeat);
end

[N_all,D] = size(X_all);
fprintf('Total windows (ALL):   %d\n', N_all);
fprintf('Num features (used):   %d\n', D);
...
tmpY       = net_B(XtrainNN(:,1));
numClasses = size(tmpY,1);
...
%% ========================================================================
%%  PART 2 – OPTIMISATION EXPERIMENTS
%% ========================================================================
...
%% 2.3 Feature-selection experiment using variability CSV
